<template>
  <div>
    <meta
      ...
      content="no-cache,no-store,must-revlidate"
    />
    ...
    <el-table
      :data="passenger_data"
      v-show="show_passenger"
      style="width: 1000px; margin-left: 200px; margin-top: 20px"
    >
      <el-table-column
        property="passenger_real_name"
        label="乘客姓名"
      ></el-table-column>
      <el-table-column
        property="passenger_phone_number"
        label="乘客电话号码"
      ></el-table-column>
      <el-table-column
        property="passenger_id_number"
        label="身份证号"
      ></el-table-column>
      <el-table-column label="操作" width="100">
        <template v-slot:default="scope">
          <el-button
            size="mini"
            type="danger"
            @click="deletepassenger(scope.row.passenger_phone_number)"
            >删除</el-button
          >
        </template>
      </el-table-column>
    ...
    >
      ...
      <el-table :data="high_seat" align="left" v-show="choose_seat === 0">
        ...
        ></el-table-column>
        <el-table-column
          property="upper_price"
          label="上铺"
          width="200"
        ></el-table-column>
        <el-table-column
          property="lower_price"
          label="下铺"
          width="200"
        ></el-table-column>
      ...
      <el-table :data="low_seat" align="left" v-show="choose_seat === 2">
        ...
        <el-table-column
          ...
          width="55"
        ></el-table-column>
        <el-table-column label="操作" width="500"> </el-table-column>
      </el-table>
    </div>
    <div
      v-show="active === 1 && dialogTableVisible_GD"
...
export default {
  ...
  methods: {
      ...
      axios
        ...
        .then((response) => {
          ...
          if (response.data.passengers.length == 0) {
            ElMessageBox.alert("无乘车人信息，请先添加乘车人", "购票失败", {
    ...
    AddPassengerInfo(
      ...
      if (
        this.passenger_data.some(
          (p) => p.passenger_id_number === passenger_id_number
        )
      ) {
        this.$message({
    ...
    handle() {
      ...
      if (this.train_no == "HSR") {
        ...
        if (this.choose_seat == 0) this.seatType = "BUS";
        else if (this.choose_seat == 1) this.seatType = "FST";
        else if (this.choose_seat == 2) this.seatType = "SND";
        axios
          .post(
            "/api/train/create_order/",
            {
              user_id: 4,
              train_name: this.train_number,
          ...
          .then((response) => {
            console.log(response);
            this.order_data = response.data.data.passenger_order_data;
            if (response.data.data.passenger_order_data.length === 0) {
            ...
            console.log(response.data.data.passenger_order_data.length);
            let order1 = {};
            order1.order_id = response.data.data.order_id;
            order1.arrival_station_name =
              response.data.data.arrival_station_name;
            ...
            order1.departure_station_name =
              response.data.data.departure_station_name;
        ...
        if (this.train_no == "REG") {
          if (this.choose_seat == 0) this.seatType = "SOF";
          else if (this.choose_seat == 1) this.seatType = "HAW";
          else if (this.choose_seat == 2) this.seatType = "HAZ";
          axios
            .post(
              "/api/train/create_order/",
              {
    ...
    deletepassenger(passenger_phone_number) {
      for (let i = 0; i < this.passenger_data.length; i++) {
        if (
          this.passenger_data[i].passenger_phone_number ==
          passenger_phone_number
        ) {
