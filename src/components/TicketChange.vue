<template>
    <div class="fillcontain">
            ...
            <el-row :gutter="20" style="margin-left: 180px;margin-top: 10px;width: 1000px">
                <el-col :span="6">
                    <div class="grid-content bg-purple">
                        <el-autocomplete disabled class="inline-input" v-model="searchForm.start_station"
                            :fetch-suggestions="query => searchElements(query, index)" placeholder="请输入始发站"
                            :trigger-on-focus="true" @select="selectedItem => handleSelect(selectedItem, index)"><template
                                v-slot="{ item }">
                                {{ item }}
                ...
                <el-col :span="6">
                    <div class="grid-content bg-purple" style="margin-left: 20px">
                        <div class="block">
                            <el-date-picker v-model="searchForm.datetime" type="date" placeholder="选择日期">
                            </el-date-picker>
                        </div>
                    </div>
                </el-col>
        ...
        <div class="table_container">
            <el-table :data="tableData" style="width: 100%" row-key="train_number">

                <el-table-column label="车次" prop="train_number">
                </el-table-column>
                <el-table-column label="出发站" prop="start_station">
                </el-table-column>
                <el-table-column label="到达站" prop="end_station">
                </el-table-column>
                ...
                <el-table-column label="二等座/硬座" prop="low_seat_count">
                </el-table-column>
        ...
        async submitForm(formName) {
            this.$refs[formName].validate(async (valid) => {
                if (valid) {
                    ...
                    axios.post('/api/train/query_train_via_station/', {
                        departure_station: this.searchForm.start_station,
                        arrival_station: this.searchForm.end_station,
                        date: `${year}-${month}-${day}`,
                    })
                        .then((response) => {
...
scope.row.train_number,scope.row.high_seat_price,scope.row.medium_seat_price,scope.row.low_seat_price) */
    ...
    created() {
        this.origin_order = { ...JSON.parse(this.$route.params.order) };
